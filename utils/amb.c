...
{
	AMB_Bus_Object base;
	AMB_Bus_Object base_night_mode;
	const char *name;
...
static void _amb_manager_property_get(char *property, E_DBus_Method_Return_Cb cb, void *data)
{
	DBusMessage *msg;
	AMB_Manager *m;
	...
	EINA_SAFETY_ON_NULL_RETURN(m->base_night_mode.path);

	if (strcmp(property, PROP_NIGHT_MODE_STRING) == 0) {
		char *property_name = PROP_NIGHT_MODE_STRING; 

		msg = dbus_message_new_method_call(
			bus_id, m->base_night_mode.path, AMB_NIGHT_MODE_IFACE, "get");
					...
					DBusError *err)
{
	...
	_amb_manager_property_get(PROP_NIGHT_MODE_STRING, _amb_get_night_mode_reply, NULL);
}
...
static void _amb_free(AMB_Manager *m)
{
	...
	_bus_object_free(&m->base_night_mode);
	free(m);
}
